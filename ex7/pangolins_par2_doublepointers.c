    ...
    while(not_finished)
    {
      if((*current_root)->tag == OBJECT)
      {
        ...
        }else
        {
          ...
          scanf("%s",yes_no);
          getchar();
          if(root_temp->tag == OBJECT)
          {
            root_temp = new_question;
            if(strcmp(yes_no, "Yes") ==0)
            {
              root_temp->yes = new_answer;
              root_temp->no = old_temp;
            }else if(strcmp(yes_no, "No") ==0)
            {
              root_temp->no = new_answer;
              root_temp->yes = old_temp;
            }
          }
          else if((*current_root)->tag == OBJECT)
          {
            printf("before overw: %s\n",(*current_root)->data.object);
