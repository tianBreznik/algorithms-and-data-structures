...
void print_instance()
{
  ...
  {
    //printf("\rIteration: %f \n", (double)i/Nitems);
    /*printf("\r%3f%%", (double)i/Nitems);
    fflush(stdout);
    if(((double)item_values[i]/(double)item_weights[i]) > ratio){
      ratio = (double)item_values[i]/(double)item_weights[i];
      ideal_index = i;
  ...
  //printf("\n");
  printf("%d\n", Capacity);
  //printf("Ideal solution is %d\t%d\t%d\nRatio: %f\n",ideal_index, weight, value, ratio);
